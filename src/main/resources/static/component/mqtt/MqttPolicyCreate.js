Vue.component('mqtt-policy-create', {
    props: {
    ...
    <div class="component mqtt-create">
        ...
        <el-row class="nav" :gutter="20">
          <el-col :span="24" class="nav-title">添加推送策略</el-col>
        </el-row> 
    ...
    methods: {
        ...
        submitForm() {
            this.$refs['form'].validate((valid) => {
                if (valid) {
                    // 调用新增接口
                    axios.post('/api/mqttPolicy/add', this.form)
                        .then(response => {
                            if(!response.data.ok){
                                this.$message.error("新增失败: " + response.data.data);
                                return
                            }
