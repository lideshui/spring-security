Vue.component('template-edit', {
    ...
    <div class="component template-create">
        ...
        <el-row class="nav" :gutter="20">
          <el-col :span="24" class="nav-title">修改设备模板</el-col>
        </el-row> 
    ...
    data() {
        return {
            templateForm: {
                ...
                pollingMethod: this.data.pollingMethod,
                communicationProtocol: this.data.communicationProtocol,
            },
            messageForm: {
                messageId: '',
                templateId: '',
    ...
    methods: {
        ...
        createTemplate() {
            this.$refs['templateForm'].validate((valid) => {
                if (valid) {
                    // 调用修改接口
                    axios.post('/api/template/edit', this.templateForm)
                        .then(response => {
                            this.$message.success("修改设备模板成功");
                            this.linkPage('template-list')
                        })
                        .catch(error => {
                            this.$message.error("修改设备模板失败: " + error.message);
                        });
                } else {
