...
public class AuthService {
    ...
    public String createToken(SysUser loginUser) {
        ...
        // 2. 设置用户代理信息（如浏览器、操作系统）
        setUserAgent(loginUser);

        // 3. 刷新用户的登录时间和过期时间，并将 loginUser 对象存入 Redis
    ...
    public void refreshToken(LoginUser loginUser) {
        ...
        // 2. 计算过期时间 (毫秒)
        loginUser.setExpireTime(loginUser.getLoginTime() + RedisConst.USER_LOGIN_TIMEOUT * 1000L); // 注意：RedisConst 是秒，这里转为毫秒
    ...
    private void setUserAgent(LoginUser loginUser) {
        // TODO: 实际实现需获取 HttpServletRequest
        loginUser.setBrowser("Unknown Client");
    }
